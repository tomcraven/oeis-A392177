...
Read [`docs/OPTIMISATION.md`](../../docs/OPTIMISATION.md) before proposing sim CPU work: it lists guardrails, verified timings, changes already in the tree, and experiments that regressed (threading, SIMD/SWAR, occupancy skips, and similar). Do not repeat failed approaches unless you have a new hypothesis and a fresh A/B plan.
...
Optional env vars: `TIME_SIM_ITERS` (default 15), `TIME_SIM_WARMUP` (default 2). Output includes mean, median, and stdev milliseconds per case. Record results in [`docs/OPTIMISATION.md`](../../docs/OPTIMISATION.md) when a change is kept or definitively rejected.
...
- Release profile / codegen / attribute tuning (`lto`, `#[inline]`, `#[cold]`, `unsafe` hot-path tricks) as the main sim CPU strategy—sim work should stay portable for WASM (see **WASM / portable sim CPU work** in [`docs/OPTIMISATION.md`](../../docs/OPTIMISATION.md)).
