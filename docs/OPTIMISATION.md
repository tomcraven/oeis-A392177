...
## What did not work (2026-05-28 perf pass — round 10)

Session baseline (post cursor-advance commit, `TIME_SIM_ITERS=25`, `TIME_SIM_WARMUP=3`, three harness passes; medians averaged): `knight_2_pairwise` 2.842, `knight_3_clique` 3.131, `leaper_4_mixed_clique` 3.107, `king_6_clique` 3.174, `chimera_3_clique` 4.527 ms. Checksums unchanged in every A/B run; `cargo testd --lib` passed.

- **Compact `u8` `OccupancyGrid` slots** — re-test of the pre-attack-layers `u32` shrink, now with the smaller 1-byte sentinel (`u8::MAX`) so the ~100k-cell occupancy buffer should fit in L1. `knight_2_pairwise` 2.990 / 3.135 / 2.957 ms (+5-6% vs baseline) and `leaper_4_mixed_clique` +10% on the noisy pass; multi-piece cases were flat at best. The hardware prefetcher already keeps the sequential scan walk hot in L2 with 8-byte slots, while reading u8 adds a sign-extension to `usize` on every `piece_id_at` / render lookup. Reverted.
- **Interleaved attack bitset (`Vec<u64>` with stride = piece count)** — replace `attack_layers: Vec<ForbiddenSet>` with a single flat row-per-word storage so `combined_forbidden_word` becomes a single cache-line read of N adjacent u64s. Helped `knight_3_clique` and `king_6_clique` (~-4%) but regressed `knight_2_pairwise` by +5-6%; switching from a slice get to per-piece checked-index reads (`words[base + p]`) still left `knight_2_pairwise` +3-5% slower. Net across cases ~ flat with large skew. The OLD `ForbiddenSet`-per-piece path already keeps each attacker's words in its own small Vec (≤ tens of KB total, fits in L1) so the cache-locality win is small, and the wider stride hurts the simplest case where there are only two attackers to OR. Reverted.
- **Stack-batched `record_forbidden` via `Vec<u32>` scratch on `Simulation`** — split the loop into "phase 1: resolve every move's `xy_to_index` into a reused Vec" then "phase 2: insert into the layer", giving the CPU explicit instruction-level parallelism on the multi-branch `xy_to_index`. Catastrophic regression (+11-35% across cases, +35% on `chimera_3_clique`). The Vec's per-push bounds check / capacity-update writes, plus the `for ... in scratch.iter()` borrow shape, defeat what the OoO core was already doing on the original tight loop where `attack_layers[piece_id].insert(...)` happened inline. Reverted. (A `[u32; 32]` stack-buffer variant ran but panicked on the random `dense_clique_like` preset because UI / random-gen patterns can produce > 32 moves; ruled out without timing.)
- **Single-element hoist `let layer = &mut self.attack_layers[piece_id]` in `record_forbidden`** — moves the per-iteration index out of the loop. Regressed every case (`chimera_3_clique` +13%, others +1-7%). The extant pattern of inline `self.attack_layers[piece_id].insert(...)` evidently gives LLVM a freer alias-noalias view than holding an explicit `&mut ForbiddenSet` across the loop body. Reverted.

**Kept (round 10):** nothing. The cursor-advance change above remains the only sim-loop win since the attack-layer split.

**Next targets still on the table (unchanged from round 9):** offline precomputed attack tables (binary-size tradeoff); piece-specific closed-form attack indexing; reductions inside `xy_to_index` itself (currently ~40% of step on `place`-heavy cases). All previous attempts at runtime tables, per-piece batching, and forbidden-representation tweaks have regressed at least one bench, so the next attempt should either be algorithmic (different placement search) or compile-time (codegen / table generation), not loop-shape micro-optims.

## What did not work (2026-05-28 perf pass — round 11, `xy_to_index` deep dive)

Baseline unchanged from round 10. Round 11 specifically targets the `xy_to_index` hot path (60-72% of `place` replay per `place_profile`).

- **Branchless 4-way side selection in `spiral::xy_to_index`** — compute all four candidate per-side ring offsets unconditionally (`off_right`, `off_top`, `off_left`, `off_bottom` as small linear combinations of `(x, y, ring)`) then layer three conditional overrides to encode the original `right > top > left > bottom` priority. The hope was that LLVM would lower the overrides into a `csel` chain and eliminate the original 4-way compare-and-branch tower. Regressed every case (`knight_2_pairwise` +12.8%, `leaper_4_mixed_clique` +17.4%, `chimera_3_clique` +11.8%). On AArch64 with our access pattern the branch predictor handles the original tower with a high hit rate — each piece's 8 attacks pick a small set of sides predictable from the piece's own (x, y) — so the always-on multiplies-by-3/5/7 plus three `csel`s cost strictly more than the 1-2 mispredicted branches per piece they replaced. Reverted.
- **`#[inline]` hint on `spiral::xy_to_index`** — the `VisitOrder::xy_to_index` wrapper is already `#[inline(always)]` but the underlying function had no inline attribute, which can leave a cross-module call through the wrapper in tight loops. Adding `#[inline]` was net flat / slight regression (`knight_2_pairwise` +1-4%, others ±1%). LLVM already inlines the body in release LTO; the explicit hint doesn't unlock further specialization because the caller's loop already has `(x, y)` and `(dx, dy)` visible. Reverted.
- **Piece-relative `record_forbidden` with hoisted `VisitOrder` dispatch and incremental ring-start cache** — branch once on `self.visit_order` outside the move loop, then in the `SquareSpiral` fast path precompute an 11-entry stack array of `ring_start` for `piece_ring ± 5` using the closed form `start[R+1] − start[R] = 8R` (a pure shift-add fill instead of squarings), and look up the start for each attack rather than recomputing `(2*ring − 1)²`. Two correctness pitfalls during prototyping: the `8R` recurrence breaks at the 0↔1 transition (`start[1] − start[0] = 1`, not 0) because ring 0 is a single cell rather than an 8-cell perimeter — handled by falling back for `piece_ring == 0` (one placement per game). Even with the bug fixed, the change regressed every case (`knight_2_pairwise` +12.5%, `leaper_4_mixed_clique` +10.4%, `king_6_clique` +6.8%, `chimera_3_clique` +2.6%). The cost model: ~30 cycles to fill the ring window per placement, plus a bounds-checked array read per attack — outweighs the ~3 cycles saved by skipping one multiply. The branch predictor on the existing tight inline-multiply path was already cheaper than the precompute+lookup tax. Reverted.

**Kept (round 11):** nothing. Three different angles of attack on `xy_to_index` (branchless cmov, attribute-only inlining hint, piece-relative cache with ring-start precompute) all regressed. The function's current shape — a clean 4-way branch over compile-time constants of `ring`, with the `(2*ring − 1)²` multiply absorbed into the same dependency chain as the offset — appears to already be near-optimal on AArch64 release codegen with our access pattern.

**Next targets still on the table:**
